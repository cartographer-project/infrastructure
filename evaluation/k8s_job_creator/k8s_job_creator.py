...
class EvaluationJob(object):
  ...
  def __init__(self, docker_img, experiment_id, tags, bag_file, platform,
               launch_pkg, offline_launch_file, assets_launch_file,
               ground_truth_file):
...
class KubernetesJobCreator(object):
  ...
  def _create_job_args(self, evaluation_job):
    eval_arguments = [
        "--dataset_path", evaluation_job.bag_file, "--platform",
        evaluation_job.platform, "--launch_file_pkg", evaluation_job.launch_pkg,
        "--launch_file", evaluation_job.offline_launch_file,
        "--assets_launch_file", evaluation_job.assets_launch_file, "--uuid",
        evaluation_job.uuid, "--experiment_id", evaluation_job.experiment_id,
  ...
  with open(csv_filename, "rb") as f:
    ...
    eval_jobs = [
        EvaluationJob(docker_img, experiment_id, tags, row["dataset"].strip(),
                      row["platform"].strip(), row["launch_pkg"].strip(),
